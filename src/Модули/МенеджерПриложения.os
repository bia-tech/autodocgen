...
// Добавляет команду в приложение на основе метаданных класса
//
// Параметры:
//	ИмяКлассаРеализации - Строка - Имя файла класса, в котором реализована команда
//
Процедура ДобавитьКомандуПриложения(Знач ИмяКлассаРеализации) Экспорт

	Попытка
		РеализацияКоманды = Новый(ИмяКлассаРеализации);
		ИмяКоманды = РеализацияКоманды.ИмяКоманды();

		Команда = ПарсерКоманд.ОписаниеКоманды(ИмяКоманды, РеализацияКоманды.ОписаниеКоманды());
		ПарсерКоманд.ДобавитьКоманду(Команда);
	...
	Исключение
		ЗавершитьРаботуПриложенияСОшибкой(СтрШаблон("Не удалось инициализировать команду из класса '%1' по причине:
		|%2", ИмяКлассаРеализации, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки;
